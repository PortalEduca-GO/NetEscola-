...
import { AnalysisData, VideoRecommendation, QuizDifficulty, QuizQuestion, Bimester } from '../types'; 
import { DEFAULT_SUBJECT_PERFORMANCE_THRESHOLD, ALL_VIDEOS, BIMESTER_OPTIONS } from '../constants'; 
...
const BIMESTER_DATE_RANGES = (year: number) => ({
  [Bimester.BIM_1]: { start: new Date(year, 0, 20), end: new Date(year, 3, 4) }, // Jan 20 - Apr 4
  [Bimester.BIM_2]: { start: new Date(year, 3, 7), end: new Date(year, 5, 30) }, // Apr 7 - Jun 30
  [Bimester.BIM_3]: { start: new Date(year, 7, 1), end: new Date(year, 9, 6) },  // Aug 1 - Oct 6
  [Bimester.BIM_4]: { start: new Date(year, 9, 7), end: new Date(year, 11, 19) }, // Oct 7 - Dec 19
});

const parseDateFromTitle = (title: string, year: number): Date | null => {
...
const AnalysisResult: React.FC<AnalysisResultProps> = ({ 
  ...
  const [selectedBimester, setSelectedBimester] = useState<Bimester>(analysisData?.bimester || BIMESTER_OPTIONS[0]);
  ...
  const filterAndPrepareVideos = useCallback(async (data: AnalysisData, bimesterOverride?: Bimester) => {
    ...
    // 2. Date-based filtering for GoiásTec videos
    const currentYear = new Date().getFullYear();
    const dateRanges = BIMESTER_DATE_RANGES(currentYear);
    const selectedBimesterRange = dateRanges[bimesterOverride || data.bimester];

    if (selectedBimesterRange) {
        baseVideoPool = baseVideoPool.filter(video => {
            if (video.source !== 'GoiásTec') {
                return true; // Keep non-GoiásTec videos
            }
            const videoDate = parseDateFromTitle(video.title, currentYear);
            if (!videoDate) {
                return true; // Keep GoiásTec videos without a parseable date
            }
            return videoDate >= selectedBimesterRange.start && videoDate <= selectedBimesterRange.end;
        });
    }

    // 3. If user is actively filtering, apply that filter to the pool.
    ...
    const sortedVideos = [...baseVideoPool].sort((a, b) => {
        const subjectsToPrioritize = (activelyFiltered && selectedFilterSubjects.size > 0)
            ? Array.from(selectedFilterSubjects).map(s => s.toLowerCase())
            : focusSubjectsLower;

        const aIsFocus = subjectsToPrioritize.includes(a.subject.toLowerCase());
        const bIsFocus = subjectsToPrioritize.includes(b.subject.toLowerCase());
  ...
  }, [difficultSubjectsList, activelyFiltered, selectedFilterSubjects, selectedBimester]);
  ...
  useEffect(() => {
    if (analysisData && analysisData.performance) {
      ...
      setSelectedBimester(analysisData.bimester);
      
      const loadInitialData = async () => {
        await filterAndPrepareVideos(analysisData, selectedBimester);
  ...
  useEffect(() => {
    if (analysisData && isComponentReady) {
      filterAndPrepareVideos(analysisData, selectedBimester);
    }
  }, [selectedBimester, analysisData, isComponentReady]);
  ...
  return (
    <div className="container mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col md:flex-row gap-6 lg:gap-8">
        ...
        >
            <div className="mb-6">
                <h4 className="text-lg font-semibold text-brandDarkGray mb-1">Selecionar Bimestre</h4>
                <p className="text-xs text-gray-500 mb-3">Escolha o bimestre para ver vídeos recomendados.</p>
                <select
                    value={selectedBimester}
                    onChange={(e) => setSelectedBimester(e.target.value as Bimester)}
                    className="w-full p-2 border border-gray-300 rounded-md focus:ring-brandGreen focus:border-brandGreen"
                    aria-label="Selecionar bimestre para recomendações"
                >
        ...
        <main className="flex-1 min-w-0">
            ...
            {!isLoadingRecommendations && recommendedVideos.length === 0 && (
              <div className="p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded text-yellow-700 mt-6">
                  ...
                  <p className="text-sm">
                      Não encontramos vídeos para os critérios selecionados ({activelyFiltered ? "filtros ativos" : "dificuldades identificadas e bimestre atual"}). 
                      Tente ajustar os filtros ou verificar outros bimestres para encontrar mais conteúdos.
                  </p>
              </div>
            )}
        </main>
